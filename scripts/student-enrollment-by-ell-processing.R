...
##################################################################
#
# Processing Script for Student-Enrollment-by-ELL
...
complete_student_enrollment_long <- reshape(complete_student_enrollment,
                                        ...
                                        v.names = "Value",
                                        timevar = "English Learner Status",
                                        times = cols_to_stack,
...
complete_student_enrollment_long$`English Learner Status`[complete_student_enrollment_long$`English Learner Status` == "Yes"] <- "English Language Learner"
complete_student_enrollment_long$`English Learner Status`[complete_student_enrollment_long$`English Learner Status` == "No"] <- "Non-English Language Learner"
...
test <- complete_student_enrollment_long[,c("District", "Year", "English Learner Status")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_student_enrollment_long,
  file.path(path_to_top_level, "data", "student_enrollment_by_ELL_2008-2016.csv"),
  sep = ",",
  row.names = F
)
